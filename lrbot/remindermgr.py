...
class ReminderManager:
    ...
    async def runActions(self, reminder: dict) -> None:
        ...
        for action in reminder['actions']:
            #try:
            await lrbot.response.runAction(action, self.client)
            # except:
            #     # Send a message to the author if an error occurred processing the reminder
            #     if 'author' in reminder:
            #         try:
            #             authorID = int(reminder['author'])
            #             reminderName = reminder['name']
            #             await lrbot.response.sendResponse(
            #                 (await self.client.fetch_user(authorID)),
            #                 f'An error has occurred while processing an action for reminder "{reminderName}". One or more actions may not have been performed.'
            #             )
            #         except:
            #             continue
        return
    ...
    async def updateNextRun(self, reminder: dict, skip: bool = False) -> dict:
        ...
        for pattern in reminder['repeat']:
            ...
            #             await lrbot.response.sendResponse(
            #                 (await self.client.fetch_user(authorID)),
            #                 'An error has occurred while processing a repeat pattern for a reminder.'
            #             )
            #         except:
            #             continue
    ...
    async def checkMessage(self, message: discord.Message) -> None:
        '''
